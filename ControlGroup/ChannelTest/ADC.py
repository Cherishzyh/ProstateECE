...
def EnsembleTrain(model_folder, save_folder):
...
def ModelSUH(model_folder, data_root, weights_list=None):
    from Metric.classification_statistics import get_auc, draw_roc
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    input_shape = (192, 192)
    batch_size = 2
    # model_folder = model_root + '/ResNeXt_CBAM_CV_20201028_dis&age&psa'
    bc = BinaryClassification()
    ...
    for cv_index, cv_folder in enumerate(cv_folder_list):
        ...
        for inputs, outputs in data_loader:
            ...
            preds = model(*inputs)[:, 1]
            if isinstance(preds.cpu().data.numpy().squeeze().tolist(), float):
...
if __name__ == '__main__':
    # EnsembleTrain('/ResNeXt_CBAM_CV_20201030_ADC')

    model_folder = r'/home/zhangyihong/Documents/ProstateECE/Model/ResNeXt_CBAM_CV_20201030_DWI&ADC'

    data_root = r'/home/zhangyihong/Documents/ProstateECE/NPYNoDivide'
    ModelTest(model_folder, data_root, weights_list=None, data_type=None)

    data_root = r'/home/zhangyihong/Documents/ProstateECE/SUH_Dwi1500'
    ModelSUH(model_folder, data_root, weights_list=None)