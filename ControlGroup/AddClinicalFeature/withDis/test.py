...
from AddClinicalFeature.withDis.ModelwithDis import ResNeXt
...
def EnsembleInference(data_type, weights_list=None):
    ...
    input_shape = (192, 192)
    batch_size = 2
    model_folder = model_root + '/ResNeXt_CBAM_CV_20201116_dis_five'
...
def ModelTest(weights_list=None, data_type=None):
    ...
    for type in data_type:
        ...
        mean_pred = np.mean(cv_pred, axis=0)
        mean_label = np.mean(cv_label, axis=0)
        np.save(os.path.join(r'/home/zhangyihong/Documents/ProstateECE/Result/NPY/AddClinical/FiveFeaturebGS',
                             type+'JSPH_dis&five_label.npy'), mean_label)
        np.save(os.path.join(r'/home/zhangyihong/Documents/ProstateECE/Result/NPY/AddClinical/FiveFeaturebGS',
                             type+'JSPH_dis&five_pred.npy'), mean_pred)
...
def ModelSUH(weights_list=None):
    ...
    mean_pred = np.mean(cv_pred, axis=0)
    mean_label = np.mean(cv_label, axis=0)
    np.save(os.path.join(r'/home/zhangyihong/Documents/ProstateECE/Result/NPY/AddClinical/FiveFeaturebGS', 'SUH_dis&five_label.npy'),
            mean_label)
    np.save(os.path.join(r'/home/zhangyihong/Documents/ProstateECE/Result/NPY/AddClinical/FiveFeaturebGS', 'SUH_dis&five_pred.npy'),
            mean_pred)
