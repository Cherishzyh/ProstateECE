...
from AddClinicalFeature.withDisLastfc.ModelwithDis import ResNeXt
...
def ModelTest(weights_list=None, data_type=None):
    ...
    for type in data_type:
        ...
        mean_pred = np.mean(cv_pred, axis=0)
        mean_label = np.mean(cv_label, axis=0)
        np.save(os.path.join(r'/home/zhangyihong/Documents/ProstateECE/Result/NPY/AddClinical/FiveFeatureLFbGS',
...
def ModelSUH(weights_list=None):
    ...
    mean_pred = np.mean(cv_pred, axis=0)
    mean_label = np.mean(cv_label, axis=0)
    np.save(os.path.join(r'/home/zhangyihong/Documents/ProstateECE/Result/NPY/AddClinical/FiveFeatureLFbGS', 'SUH_FiveFeature_label.npy'),
            mean_label)
    np.save(os.path.join(r'/home/zhangyihong/Documents/ProstateECE/Result/NPY/AddClinical/FiveFeatureLFbGS', 'SUH_FiveFeature_pred.npy'),
            mean_pred)
