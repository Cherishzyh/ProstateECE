...
from MyModel.Block import DoubleConv
...
class MultiTaskModel(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # ResNet
        self.resnet34 = ResNet(Bottleneck, [3, 4, 6, 3])
    ...
    def forward(self, x):
        ...
        c9b = self.conv9b(c8b)
        outb = self.sigmoidb(c9b)

        class_inputs = torch.cat((x, outa, outb), dim=1)
        outc, feature_map = self.resnet34(class_inputs)

        return outa, outb, outc, feature_map


class MultiSegModel(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(MultiSegModel, self).__init__()
