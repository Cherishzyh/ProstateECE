...
from Model.Block import AttentionBlock3D, ResidualBlock, conv3x3
...
class AttenUNet2DDiffLr(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(AttenUNet2DDiffLr, self).__init__()
...
class AttenUNetDiffLr(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(AttenUNetDiffLr, self).__init__()
        self.EncodingResBlock1 = ResidualBlock(in_channels, 64)
        self.EncodingPool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.EncodingResBlock2 = ResidualBlock(64, 128)
        self.EncodingPool2 = nn.MaxPool2d(kernel_size=2, stride=2)
...
def test():
    ...
    model = AttenUNetDiffLr(in_channels=1, out_channels=1)
    # print(model)
